...
class yaOs
{
   ...
   *
   * yaOs("/home/user/", "/dir", "sub_dir", "file.ext")
  ...
  public static function strictPath($path)
  {
    if ('/' !== DIRECTORY_SEPARATOR)
    {
      $path = str_replace('/', DIRECTORY_SEPARATOR, $path);
  ...
  {
    $random = yaString::random(4);
    $ext = self::getFileExtension($file, true);
    $strip_len = $maxLen - strlen($random) - strlen($ext);

    $strip_name = yaString::truncate(self::sanitizeFileName(self::getFileWithoutExtension($file)), $strip_len, "");
  ...
  public static function randomizeDirName($dir, $maxLen = 255)
  {
    ...
    $strip_name = yaString::truncate(self::sanitizeDirName($file), $strip_len, "");
  ...
  {
    return trim(preg_replace('/[^\w\._-]+/i', '-', yaString::transliterate($dirName)), '-');
  }
  ...
  public static function getPerformanceInfo()
  {
    return array(
      ...
      'time'  => round(1000*(microtime(true) - ya::getStartTime()))
    );
  }
