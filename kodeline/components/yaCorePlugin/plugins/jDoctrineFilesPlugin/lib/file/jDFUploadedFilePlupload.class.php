<?php
/**
 * jDFUploadedFile is a class for dealing with uploads.
 * 
 * Developes for plupload.
 * Class for base is sfPluploadUploadedFile (sfPluploadPlugin)
 *
 * @package     jDoctrineFilesPlugin
 * @subpackage  lib
 * @author      Chugarev Alexey  <chugarev@gmail.com>
 */
class jDFUploadedFilePlupload extends  jDFUploadedFile
{
  /**
   * @var int
   */
  ...
  public function __construct($chunk, $chunks, $filename)
  {
    $this
      ...
      ->setTargetDir(
          sys_get_temp_dir()
          . (substr(sys_get_temp_dir(), -1) == DIRECTORY_SEPARATOR ? '' : DIRECTORY_SEPARATOR)
   ...
   * @throws  Exception
   */
  public function processUpload(array $file, $contentType, $cleanUpOldFiles = true)
  {
    $this->_initTimeLimit();
  ...
  protected function _processMultipart(array $file)
  {
    ...
    $outputFile = fopen($this->getFilePath(), ($this->getChunk() == 0 ? 'wb' : 'ab'));
