<?php
abstract class BaseBackendKlCmsUserGroupActions extends yaBaseActions
{
  /**
   * Default action.
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    $query = Doctrine::getTable('klUserGroup')->createQuery('klu');

    // Define configuration for pager.
    $arPagerConfig = sfConfig::get('app_klCmsUsersPlugin_klCmsUserGroup_per_page', array('per_page' => 10));

    // Define pager.
    $this->items_pager = new klDoctrinePager('klUserGroup', $arPagerConfig['per_page']);
    $this->items_pager->setQuery($query);
    $this->items_pager->setPage($request->getParameter('page', 1));
  ...
  /**
   * Action for add new usergroup.
   * @param sfWebRequest $request Web request.
   */
  public function executeNew(sfWebRequest $request)
  {
    // Initiate form object.
    $this->form = new klUserGroupForm();
  ...
  public function executeEdit(sfWebRequest $request)
  {
    ...
    $this->forward404Unless(
      ($this->id_user = $request->getParameter('id_usergroup', null)), 
      $this->getContext()->getI18N()->__('ID группы не был указан!', null, 'klCmsUsersPlugin'));

    // Fetch user by id.
    $this->object = Doctrine::getTable('klUserGroup')->createQuery()->where('id = ?', $this->id_user)->fetchOne();

    // Throw exception if object is not found.
    $this->forward404Unless($this->object, 
      sprintf($this->getContext()->getI18N()->__('Группа с id=%d не найдена!', null, 'klCmsUsersPlugin'), $this->id_user));

    // Initiate form object.
    $this->form = new klUserGroupForm($this->object);
