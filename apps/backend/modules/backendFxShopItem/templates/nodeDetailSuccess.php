...
<table id="rounded-corner">
  ...
  <tbody>
    ...
    <tr>
      ...
      <?php endif ?>
      <td>
        <a href="<?php echo url_for('@backend_product_item_node_position?motion=up&id=' . $item['id']) ?>"><img src="/backend/images/arrow_up.png" alt="" title="" border="0" /></a>
        <a href="<?php echo url_for('@backend_product_item_node_position?motion=down&id=' . $item['id']) ?>"><img src="/backend/images/arrow_down.png" alt="" title="" border="0" /></a>
      </td>
      <td>       
        <?php if ($item['is_category']): ?>
        <a href="<?php echo url_for('@backend_product_category_new?parent_id=' . $item['id']) ?>"><img src="/backend/images/add_subfolder.png" alt="Добавить подкатегорию" title="Добавить подкатегорию" border="0" /></a>
        <a href="<?php echo url_for('@backend_product_category_edit?id=' . $item['id']) ?>"><img src="/backend/images/edit_item.png" alt="Редактировать" title="Редактировать" border="0" /></a>
        <?php else: ?>
        <a href="<?php echo url_for('@backend_product_item_node_edit?id=' . $item['id']) ?>"><img src="/backend/images/edit_item.png" alt="Редактировать" title="Редактировать" border="0" /></a>
        <?php endif ?>

        <a href="<?php echo url_for('@parameterable_component_parameters?object_id=' . $item['id'] . '&model=' . $modelName) ?>"><img src="/backend/images/parameters.png" alt="Список параметров элемента" title="Список параметров элемента" border="0" /></a>
        <a href="<?php echo url_for('@backend_product_item_node_delete?id=' . $item['id']) ?>" class="ask"><img src="/backend/images/trash.png" alt="Удалить" title="Удалить" border="0" /></a>
      </td>
    </tr>
    <?php endforeach ?>
    <?php endif ?>
